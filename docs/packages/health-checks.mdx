...
In this example, I'm using Prisma ORM and built indicator above prisma connection. First, install `@tinybudgie/health-check` package
...
```ts title="libs/sample/src/lib/sample-indicators/sample-prisma-connection.health-indicator.ts"
...
```ts title="libs/sample/src/lib/sample-services/sample-prisma.service.ts"
...
````mdx-code-block
<BrowserWindow url="http://localhost:3000/health" minHeight={240}>
...
[Don't trust, verify!](https://github.com/temarusanov/nx/tree/main/libs/external/health-checks)
